...
def show_pause_menu(screen):
    ...
    # Charger les images des boutons
    button_resume_img = pygame.image.load(path + "resume_button.png")
    button_quit_img = pygame.image.load(path + "exit_button.png")
    
    button_resume_img = pygame.transform.scale(button_resume_img, (button_resume_img.get_width() // 10, button_resume_img.get_height() // 10))
    button_quit_img = pygame.transform.scale(button_quit_img, (button_quit_img.get_width() // 10, button_quit_img.get_height() // 10))

    # Définir les positions des boutons
    button_resume_rect = button_resume_img.get_rect(center=(conf_screen.WIDTH_SCREEN*(2/9), conf_screen.HEIGHT_SCREEN*(9/10)))
    button_quit_rect = button_quit_img.get_rect(center=(conf_screen.WIDTH_SCREEN*(7/9), conf_screen.HEIGHT_SCREEN*(9/10)))
    ...
    # Boucle pour gérer le menu de pause
    while True:
        
        mouse_pos = pygame.mouse.get_pos()
        # Appliquer l'effet d'assombrissement si la souris survole un bouton
        if check_button_hover(mouse_pos, button_resume_rect):
            resume_img = darken_image(button_resume_img, DARKEN_FACTOR)
        else:
            resume_img = button_resume_img
        ...
        # Dessiner l'image de fond et les boutons
        screen.blit(background, (0, 0))  # Dessiner l'image de fond
        screen.blit(resume_img, button_resume_rect)  # Dessiner le bouton Reprendre (assombri ou non)
        screen.blit(quit_img, button_quit_rect)  # Dessiner le bouton Quitter (assombri ou non)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                # Vérifier si on clique sur "Reprendre"
                if check_button_click(mouse_pos, button_resume_rect):
                    print("Reprise du jeu")
                    return False  # Retourne une valeur pour indiquer que le jeu doit reprendre
